...
const topIndex = content.indexOf('    if (!leadId || followUps.has(leadId)) {');
const bottomIndex = content.indexOf('async function moduleQuery(');

const topPart = content.substring(0, topIndex);
const bottomPart = content.substring(bottomIndex);

const middlePart = `    if (!leadId || followUps.has(leadId)) {
      continue;
    }
...
fs.writeFileSync('c:/laragon/www/sogoerp/lib/erp-queries.ts', topPart + middlePart + bottomPart);
